...
namespace CodeGenerator
{
    internal static class Program
    {
        static void Main(string[] args)
        {
            ...
            // foreach (var funcs in defs.Functions)
            // {
            //     Console.WriteLine("function name:  " + funcs.Name);
            // }
            ...
            foreach (TypeDefinition td in defs.Types)
            {
                ...
                {
                    string typeStr = GetTypeAsString(field.Type, field.IsFunctionPointer);
                ...
                foreach (TypeReference field in td.Fields)
                {
                    ...
                    {
                        string vectorElementType = GetTypeAsString(field.TemplateType, false);
                ...
                foreach (FunctionDefinition fd in defs.Functions)
                {
                    foreach (OverloadDefinition overload in fd.Overloads)
                    {
                        ...
                        for (int i = 0; i < overload.Parameters.Length; i++)
                        {
                            ...
                            string paramType = GetTypeAsString(p.Type, p.IsFunctionPointer);
                            if (p.Name == "...") { continue; }
            ...
            using (CSharpCodeWriter writer = new CSharpCodeWriter(Path.Combine(outputPath, $"{classPrefix}Native.gen.cs")))
            {
                ...
                foreach (FunctionDefinition fd in defs.Functions)
                {
                    foreach (OverloadDefinition overload in fd.Overloads)
                    {
                        ...
                        string ret = GetTypeAsString(overload.ReturnType, false);

                        bool hasVaList = false;
                        List<string> paramParts = new List<string>(overload.Parameters.Length);
                        ...
                        string methodName = isUdtVariant
                            ? exportedName.Substring(0, exportedName.IndexOf("_nonUDT", StringComparison.Ordinal))
                            : exportedName;
            ...
            using (CSharpCodeWriter writer = new CSharpCodeWriter(Path.Combine(outputPath, $"{classPrefix}.gen.cs")))
            {
                ...
                foreach (FunctionDefinition fd in defs.Functions)
                {
                    ...
                    foreach (OverloadDefinition overload in fd.Overloads)
                    {
                        var exportedName = overload.ExportedName.AsSpan();
                        
                        if (exportedName.StartsWith("ig"))
                            exportedName = exportedName.Slice(2, exportedName.Length - 2);
                        ...
                        for (int i = overload.DefaultValues.Count; i >= 0; i--)
                        {
                            ...
                            Dictionary<string, string> defaults = new Dictionary<string, string>(i);
            ...
            foreach (var method in defs.Variants)
            {
                ...
                {
                    if (!variant.Used) Console.WriteLine($"Error: Variants targeting parameter {variant.Name} with type {variant.OriginalType} could not be applied to method {method.Key}.");
                }
            }
        }
            ...
            string classPrefix)
        {
            ...
            string nativeRet = GetTypeAsString(overload.ReturnType, false);
            ...
            int overloadedParamsLen = overload.Parameters.Length;
            List<string> invocationArgs = new List<string>(overloadedParamsLen);
            MarshalledParameter[] marshalledParameters = new MarshalledParameter[overloadedParamsLen];
            List<string> preCallLines = new List<string>(overloadedParamsLen);
            List<string> postCallLines = new List<string>(overloadedParamsLen);
            
            int selfIndex = -1;
            int pOutIndex = -1;
            string overrideRet = null;
            
            for (int i = 0; i < overloadedParamsLen; i++)
            {
                ...
                string nativeTypeName = GetTypeAsString(tr.Type, tr.IsFunctionPointer);
                ...
                else switch (tr.Type)
                {
                    ...
                    default:
                    {
                        if (GetWrappedType(tr.Type, out string wrappedParamType)
                        ...
                        else if ((tr.Type.EndsWith("*") || tr.Type.Contains("[") || tr.Type.EndsWith("&")) && tr.Type != "void*" && tr.Type != "ImGuiContext*" && tr.Type != "ImPlotContext*"&& tr.Type != "EditorContext*")
                        {
                            ...
                            if (tr.Type.Contains("["))
                            {
                                ...
                                nonPtrType = GetTypeAsString(wellKnown.Substring(0, wellKnown.Length - 1), false);
                            }
                            else
                            {
                                nonPtrType = GetTypeAsString(tr.Type.Substring(0, tr.Type.Length - 1), false);
                            }
                            string nativeArgName = "native_" + tr.Name;
            ...
            List<string> nativeInvocationArgs = new List<string>(marshalledParameters.Length);
            ...
            for (int i = 0; i < marshalledParameters.Length; i++)
            {
                ...
                if (selfIndex == i)
                {
                    ...
                    string tstr = GetTypeAsString(tr.Type, false);
                ...
                if (mp.IsPinned)
                {
                    string nativePinType = GetTypeAsString(tr.Type, false);
            ...
            var targetName = overload.ExportedName.AsSpan();
            
            if (targetName.Contains("nonUDT", StringComparison.OrdinalIgnoreCase))
            {
                targetName = targetName[..targetName.IndexOf("_nonUDT")];
            }
        ...
        {
            return nativeRet switch
            {
                "bool" => "bool",
                "char*" => "ReadOnlySpan<char>",
                "ImWchar*" or "void*" => "nint",
                _ => GetTypeAsString(nativeRet, false)
            };
        }
        ...
        private static string GetTypeAsString(string typeName, bool isFunctionPointer)
        {
            int pointerLevel = 0;
        ...
        private static string CorrectIdentifier(string identifier) => 
            TypeInfo.IdentifierReplacements.TryGetValue(identifier, out string replacement) ? replacement : identifier;
    }
